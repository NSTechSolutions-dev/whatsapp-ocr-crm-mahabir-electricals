...
/**
 * Format stored WhatsApp CRM content for display / new outbound rows.
 * Supports legacy `template | var1 | var2`, `Quotation QT-…`, and already-rendered bodies.
 */
export function formatWhatsappMessageContent(content: string): string {
  const text = (content || "").trim();
  if (!text) return text;

  const quotationMatch = text.match(/^Quotation\s+(.+)$/i);
  if (quotationMatch) {
    return renderWhatsappTemplate("mahabir_quotation_pdf_delivery", [quotationMatch[1].trim()]);
  }

  if (text.includes("|")) {
    const parts = text.split("|").map((part) => part.trim()).filter(Boolean);
    const [title, ...vars] = parts;
    if (title && findTemplateBody(title)) {
      return renderWhatsappTemplate(title, vars);
    }
  }

  // Already human-readable / unknown text
  return text;
}
