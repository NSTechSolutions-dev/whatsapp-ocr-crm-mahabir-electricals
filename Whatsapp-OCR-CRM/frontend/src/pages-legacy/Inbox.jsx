...
function SimulateInboundDialog({ onCreated }) {
  ...
  const submit = async () => {
    setSubmitting(true);
    try {
      const body = { phone, name, type, content };
      ...
      setOpen(false);
      onCreated && onCreated();
      if (r.data?.conversationId) navigate(`/inbox/${r.data.conversationId}`);
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-surface border-line">
        <DialogHeader>
          <DialogTitle className="font-display">Simulate incoming WhatsApp</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          <Button onClick={submit} disabled={submitting} className="bg-brand hover:bg-brand-hover" data-testid="simulate-submit-button">
